...
pub use cognitive::CogLoop;

/// Initialize the global storage backend.
pub fn storage_init<P: AsRef<std::path::Path>>(dir: P) -> anyhow::Result<()> {
    storage::init_storage(dir)
}
...
impl MCPKernel {
    ...
    pub fn with_config(config: config::KernelConfig) -> Self {
        // Initialize tracing (try_init to avoid panic if already set)
