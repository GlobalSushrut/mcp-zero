...
mod tests {
    ...
    #[test]
    fn test_prohibited_name_terms() {
        let tree = EthicalBinaryTree::new();
        let terms = ["malware", "exploit", "hack", "attack", "virus"];
        
        for term in terms {
            let config = AgentConfig {
                name: format!("my_{}_agent", term),
                entry: None,
                intents: vec![],
                hm: Default::default(),
                metadata: Default::default(),
            ...
            assert!(
                tree.validate_spawn(&config).is_err(),
                "Should reject agent name containing '{}'", term
            );
        }
    }
    
    #[test]
    fn test_benign_agent_allowed() {
        let tree = EthicalBinaryTree::new();
        let config = AgentConfig {
            name: "weather_monitor".to_string(),
            entry: None,
            intents: vec!["read_sensor".to_string(), "report".to_string()],
            hm: Default::default(),
            metadata: Default::default(),
        };
        assert!(tree.validate_spawn(&config).is_ok());
    ...
    #[test]
    fn test_harmful_intent_with_safe_name_allowed_by_tree() {
        let tree = EthicalBinaryTree::new();
        // The ethical tree allows harmful intents if consent+legal checks pass.
        // Only the name-based check blocks at spawn time.
        let config = AgentConfig {
            name: "safe_name".to_string(),
            entry: None,
            intents: vec!["attack_network".to_string()],
            hm: Default::default(),
            metadata: Default::default(),
        };
        // Tree path: is_harmful=true → has_consent=true → is_legal=true → Allow
        assert!(tree.validate_spawn(&config).is_ok());
    }
    
    #[test]
    fn test_attack_in_name_rejected() {
        let tree = EthicalBinaryTree::new();
        let config = AgentConfig {
            name: "attack_bot".to_string(),
            entry: None,
            intents: vec!["scan".to_string()],
            hm: Default::default(),
            metadata: Default::default(),
        };
        assert!(tree.validate_spawn(&config).is_err());
    }
    
    #[test]
    fn test_execution_prohibited_actions() {
        let tree = EthicalBinaryTree::new();
        let agent_id = "agent_123".to_string();
        
        let prohibited = ["delete_all", "format", "wipe", "destroy"];
        for action in prohibited {
            assert!(
                tree.validate_execution(&agent_id, action).is_err(),
                "Should reject execution of '{}'", action
            );
        }
    }
    
    #[test]
    fn test_execution_safe_actions_allowed() {
        let tree = EthicalBinaryTree::new();
        let agent_id = "agent_123".to_string();
        
        assert!(tree.validate_execution(&agent_id, "read_sensor").is_ok());
        assert!(tree.validate_execution(&agent_id, "compute").is_ok());
        assert!(tree.validate_execution(&agent_id, "report").is_ok());
    }
    
    #[test]
    fn test_recovery_always_allowed() {
        let tree = EthicalBinaryTree::new();
        assert!(tree.validate_recovery(&"any_agent".to_string()).is_ok());
    }
    
    #[test]
    fn test_add_custom_rule() {
        let tree = EthicalBinaryTree::new();
        assert!(tree.add_rule(
            &"root".to_string(),
            "custom_check",
            "custom_rule",
            Some(Decision::Allow),
    ...
    #[test]
    fn test_add_rule_invalid_parent() {
        let tree = EthicalBinaryTree::new();
        assert!(tree.add_rule(
    ...
    fn test_default_trait() {
        let tree = EthicalBinaryTree::default();
        // Should work the same as new()
        let result = tree.evaluate("test", &serde_json::json!({}));
