...
mod tests {
    ...
    fn test_trace_id_format() {
        let tracer = PoseidonTracer::new();
        let trace_id = tracer.begin_trace(&"a1".to_string(), "intent").unwrap();
        assert!(trace_id.starts_with("trace_"));
        assert_eq!(trace_id.len(), 22); // "trace_" + 16 hex chars
    }
    
    #[test]
    fn test_trace_failed() {
        let tracer = PoseidonTracer::new();
        let agent_id = "fail_agent".to_string();
        
        let trace_id = tracer.begin_trace(&agent_id, "risky_op").unwrap();
        tracer.end_trace(&trace_id, false, Some(&serde_json::json!({"error": "boom"}))).unwrap();
        
        let proof = tracer.export_zk_proof(&trace_id).unwrap();
        assert_eq!(proof["entries"], 2); // begin + end
    }
    
    #[test]
    fn test_trace_multiple_events() {
        let tracer = PoseidonTracer::new();
        let agent_id = "multi_agent".to_string();
        
        let trace_id = tracer.begin_trace(&agent_id, "pipeline").unwrap();
        ...
        tracer.end_trace(&trace_id, true, None).unwrap();
        
        let proof = tracer.export_zk_proof(&trace_id).unwrap();
        assert_eq!(proof["entries"], 7); // begin + 5 events + end
    }
    
    #[test]
    fn test_trace_hash_chain_integrity() {
        let tracer = PoseidonTracer::new();
        let agent_id = "chain_agent".to_string();
        
        let trace_id = tracer.begin_trace(&agent_id, "chain_test").unwrap();
        tracer.record_event(&agent_id, "ev1", &serde_json::json!({})).unwrap();
        tracer.end_trace(&trace_id, true, None).unwrap();
        
        // Verify entries form a hash chain
        let entries = tracer.entries.read().unwrap();
        let chain: Vec<&TraceEntry> = entries.iter().filter(|e| e.id == trace_id).collect();
        
        assert!(chain[0].prev_hash.is_none(), "First entry has no prev_hash");
        for i in 1..chain.len() {
            assert!(chain[i].prev_hash.is_some(), "Entry {} must have prev_hash", i);
        }
    }
    
    #[test]
    fn test_trace_end_nonexistent() {
        let tracer = PoseidonTracer::new();
        let result = tracer.end_trace(&"nonexistent_trace".to_string(), true, None);
        assert!(result.is_err());
    }
    
    #[test]
    fn test_export_nonexistent_trace() {
        let tracer = PoseidonTracer::new();
        let result = tracer.export_zk_proof(&"nonexistent".to_string());
        assert!(result.is_err());
    }
    
    #[test]
    fn test_record_event_auto_creates_trace() {
        let tracer = PoseidonTracer::new();
        let agent_id = "auto_trace_agent".to_string();
        
        // No trace started, record_event should auto-create one
        tracer.record_event(&agent_id, "orphan_event", &serde_json::json!({})).unwrap();
        
        let entries = tracer.entries.read().unwrap();
        assert!(entries.len() >= 2); // auto begin + event
    }
    
    #[test]
    fn test_default_trait() {
        let tracer = PoseidonTracer::default();
        let trace_id = tracer.begin_trace(&"d".to_string(), "x").unwrap();
        assert!(trace_id.starts_with("trace_"));
    }
}
